...
class AjioLiveAdapter(BasePlatformAdapter):
    def __init__(self):
        super().__init__("Ajio")

    def search(self, query: str) -> list:
        print(f"[{self.platform_name}] Attempting to fetch live data for '{query}'...")
        live_results = self._scrape_ajio(query)
    ...
    def _scrape_ajio(self, query: str) -> list:
        ...
        try:
            with sync_playwright() as p:
                ...
                items = page.locator('.item.rilrtl-products-list__item').all()
                for item in items[:5]:
                    try:
                        brand_loc = item.locator('.brand')
                        brand = brand_loc.inner_text().strip() if brand_loc.count() > 0 else "Ajio"
                        
                        name_loc = item.locator('.nameCls')
                        if name_loc.count() == 0: continue
                        name = name_loc.inner_text().strip()
                        
                        price_loc = item.locator('.price')
                        price = 0.0
                        if price_loc.count() > 0:
                            p_text = price_loc.first.inner_text().replace('₹', '').replace(',', '').strip()
                            if p_text.isdigit(): price = float(p_text)
                            
                        if price == 0.0: continue
                        
                        orig_loc = item.locator('.orginal-price')
                        ...
                        link_loc = item.locator('a.rilrtl-products-list__link')
                        product_url = "https://www.ajio.com" + link_loc.get_attribute('href') if link_loc.count() > 0 else url
                        ...
                        print(f"Error parsing Ajio item: {e}")
                        continue
