...
class FlipkartLiveAdapter(BasePlatformAdapter):
    ...
    def _scrape_flipkart(self, query: str) -> list:
        results = []
        try:
            with sync_playwright() as p:
                ...
                try:
                    page.wait_for_selector('div[data-id]', timeout=2000)
                except:
                    browser.close()
                    return []
                
                items = page.locator('div[data-id]').all()
                for item in items[:5]:
                    try:
                        name_loc = item.locator('.KzDlHZ, .s1Q9rs, .WKTcLC')
                        if name_loc.count() == 0: continue
                        name = name_loc.first.inner_text().strip()
                        
                        price_loc = item.locator('.Nx9bqj')
                        price = 0.0
                        ...
                        img_loc = item.locator('img.DByuf4, img')
                        image_url = img_loc.first.get_attribute('src') if img_loc.count() > 0 else ""
                        
                        link_loc = item.locator('a.CGtC98, a.VJA3rP, a')
                        product_url = "https://www.flipkart.com" + link_loc.first.get_attribute('href') if link_loc.count() > 0 else url
